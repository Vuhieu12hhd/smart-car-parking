...
const char* ssid = "Home";         // Tên của mạng WiFi mà bạn muốn kết nối đến
...
void connectWebSocket() {
  webSocket.begin("192.168.43.19", 3000, "/");          // Địa chỉ websocket server, port và URL
...
//      Serial.printf("[WSc] get text: %s\n", payload);
      controlBR = String((char*)(payload));
//      Serial.println(String("Control: ") + controlBR + controlBR.length());
      if (controlBR.length() == 5) {
        statusFromSV = "*";
      }
      else if (controlBR.length() == 6) {
        statusFromSV = ")";
      }
...
//  Serial.println(statusFromSV);
//  Serial.flush();
  mySerial.println(statusFromSV);
  mySerial.flush();
}
